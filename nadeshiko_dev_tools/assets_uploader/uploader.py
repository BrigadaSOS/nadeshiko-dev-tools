...
from nadeshiko.api.media.internal import (
    media_create,
    media_update,
    episode_create,
    episode_update,
...
from nadeshiko.models.error import Error
from nadeshiko.models.media_create_request import MediaCreateRequest
from nadeshiko.models.media_create_request_category import MediaCreateRequestCategory
from nadeshiko.models.media_create_request_storage import MediaCreateRequestStorage
from nadeshiko.models.media_update_request import MediaUpdateRequest
from nadeshiko.models.media_update_request_storage import MediaUpdateRequestStorage
from nadeshiko.models.episode_create_request import EpisodeCreateRequest
from nadeshiko.models.segment_create_request import SegmentCreateRequest
from nadeshiko.models.segment_create_request_storage import SegmentCreateRequestStorage
...
class NadeshikoUploader:
    ...
    def _is_rate_limit_error(self, error: Error) -> bool:
        """Check if an error is a 429 rate limit error."""
        return getattr(error, 'status', None) == 429

    def _get_retry_after_seconds(self, error: Error) -> int:
    ...
    def _update_media_info(self, existing_media, media_info: MediaInfo, media_folder: Path) -> int | None:
        ...
        update_result = self.media_update.sync(
            client=self.api_client, media_id=existing_media.id, body=request
        )

        if update_result and not isinstance(update_result, self.Error):
            console.print(f"[green]Updated media: {existing_media.id}[/green]")
            return existing_media.id

        if isinstance(update_result, self.Error):
            console.print("\n[red]Failed to update media:[/red]")
            for attr_name in ['detail', 'status', 'code', 'title', 'errors']:
                try:
                    value = getattr(update_result, attr_name)
                    if value:
                        console.print(f"  [red]{attr_name.capitalize()}:[/red] {value}")
    ...
    def _get_or_create_media(self, media_info: MediaInfo, media_folder: Path) -> int | None:
        ...
        existing_media = None
        if result and not isinstance(result, self.Error):
        ...
        # If media exists and we're in update-info-only mode, update it
        if existing_media and self.update_info_only:
            return self._update_media_info(existing_media, media_info, media_folder)

        # If media exists and we're not in update-info-only mode, just return ID
        if existing_media:
            return existing_media.id
        ...
        if create_result and not isinstance(create_result, self.Error):
            console.print(f"[green]Created new media: {create_result.id}[/green]")
            return create_result.id

        if isinstance(create_result, self.Error):
            console.print("\n[red]Failed to create media:[/red]")
            for attr_name in ['detail', 'status', 'code', 'title', 'errors']:
                try:
                    value = getattr(create_result, attr_name)
                    if value:
                        console.print(f"  [red]{attr_name.capitalize()}:[/red] {value}")
                except (KeyError, AttributeError):
                    pass
    ...
    def _create_segment(
        ...
        for attempt in range(max_retries):
            ...
            if isinstance(result, self.Error) and getattr(result, 'status', None) == 409:
                # Segment already exists, skip silently
                return True

            # Check if this is a rate limit error that we can retry
            if isinstance(result, self.Error) and self._is_rate_limit_error(result):
                # Set global rate limit - all threads will wait for 60 seconds
                _set_rate_limit()
                self._print(
            ...
            if isinstance(result, self.Error):
                ...
                # Use try-except because Error.__getattr__ may raise KeyError directly
                for attr_name in ['detail', 'status', 'code', 'title', 'errors']:
                    try:
                        value = getattr(result, attr_name)
                        if value:
                            if attr_name == 'errors' and isinstance(value, dict):
                                self._print("  [red]Validation errors:[/red]")
                                for field, field_errors in value.items():
                                    self._print(f"    [yellow]{field}:[/yellow] {field_errors}")
                            else:
